...
def subgradecount2(rawlines,rawmark):
	global subcode2
	subcode2=[]
	for codel in rawlines:
		stucode2=[]
		for i in codel[1::]:
			if i.isdigit():
				stucode2.append(i)
		subcode2.append(stucode2)
	
	d={}
	for i in range(len(subcode2)):
		for j in range(len(subcode2[i])):

			if len(rawmark[i])!=2*len(subcode2[i]):
				continue
			else:
				if subcode2[i][j] in d:
					d[subcode2[i][j]]+=[rawmark[i][j*2+1]]
				else:
					d[subcode2[i][j]]=[rawmark[i][2*j+1]]
	return d

subgradecount2(rawlines,rawmark)
...
def stusubs(rawlines):
	...
	return stu_subjects
finalline=[]
tempmark=list(rawmark.copy())
templines=list(rawlines.copy())
tempsubcode=subcode.copy()
code=list(D.keys())
def final(lines,mark):
	...
	for line in range(len(lines)):
		...
		mark_sum=0
		mark_sum2=0
		for m in range(len(tempmark[line])):
			
			if tempmark[line][m].isdigit():
				beta = tempsubcode[line][m//2]
				if beta in ['049','028']:
					 tempsubcode[line].remove(beta)
					 tempmark[line].remove(tempmark[line][m])
					 tempmark[line].remove(tempmark[line][m])
					 break
		for m in tempmark[line]:
			 if m.isdigit():
			 	mark_sum+=int(m)
		percentage=mark_sum/(5)
		l.append(mark_sum)
		l.append(percentage)
		finalline.append(l)
	return finalline
...
cvf5=open('rank_wise_analysis.csv','w')
writer3=csv.writer(cvf5)
...
head.append('TOTAL MARKS (MAIN 5)')
head.append('PERCENTAGE (MAIN 5)')
...
cvf4=open('topper_data.csv','w')
writer=csv.writer(cvf2)
writer2=csv.writer(cvf4)
glist=['A1','A2','B1',"B2","C1","C2","D1","D2","E"]
head=['CODE','SUBJECT','NO. OF STUDENTS','APPREARED','ABSENT','PASS','FAIL','PASS PERCENT','P.I.']+glist+[ 'HIGHEST SCORE', 'LOWEST SCORE', 'AVERAGE SCORE' ]
writer.writerow(head)
if '301' in code:
	sub=subjects
elif '184' in code:
...
for i in code:
	...
	for grade in D[i]:
		if grade=='E':
			fail+=1
	data=[i, sub[i], no_of_students,sum(count(D,i).values()),0-sum(count(D,i).values())+no_of_students, (sum(count(D,i).values())-fail),fail,((sum(count(D,i).values())-fail)*100)/sum(count(D,i).values()), subPI(D,i)]+list(count(D,i).values())+[max(filtered_submarks[i]),min(filtered_submarks[i]), sum(filtered_submarks[i])/len(filtered_submarks[i])]
...
cvf3=open('stream_wise_and_overall_analysis.csv','w')
writer=csv.writer(cvf3)
writer.writerow(['STREAM', 'TOTAL STUDENTS', 'TOTAL APPEARED', 'PASS', 'ABSENT', 'COMPARTMENT', 'ESSENTIAL REPEAT', 'PASS PERCENT', 'P.I.', 'HIGHEST MARKS', 'AVERAGE MARKS'])
writer2.writerow(['STREAM','1ST TOPPER' ,'ROLL NO.','MARKS','2ND TOPPER','ROLL NO.', 'MARKS', '3RD TOPPER', 'ROLL NO.', 'MARKS'])
if classname==12:
	for stream in ['SCIENCE', 'COMMERCE', 'HUMANITIES']:
		...
		sortedplist=sorted(percentlist)
		writer.writerow([stream, len(sw_finalline), len(sw_finalline)-abs, passcount, abs, comp, repeat, (passcount*100)/len(sw_finalline), sw_PI, max(percentlist), sum(percentlist)/len(percentlist)])
		writer2.writerow([stream, sw_finalline[percentlist.index(sortedplist[-1])][2], sw_finalline[percentlist.index(sortedplist[-1])][0], sw_finalline[percentlist.index(sortedplist[-1])][-1], sw_finalline[percentlist.index(sortedplist[-2])][2], sw_finalline[percentlist.index(sortedplist[-2])][0], sw_finalline[percentlist.index(sortedplist[-2])][-1], sw_finalline[percentlist.index(sortedplist[-3])][2], sw_finalline[percentlist.index(sortedplist[-3])][0], sw_finalline[percentlist.index(sortedplist[-3])][-1] ])
	writer.writerow([])
...
for i in remark:
	if i=='PASS':
		...
		repeat+=1
writer.writerow(['OVERALL', len(finalline),len(finalline)-abs, passcount, abs, comp, repeat, passcount*100/len(finalline), studentPI_list(rawmark), max(percentl), sum(percentl)/len(percentl)])
writer2.writerow([])
rank_list=sorted(finalline, key=lambda x: x[-1])[::-1]
for i in rank_list:
	writer3.writerow(i)
cvf5.close()
Z=['OVERALL',  finalline[percentl.index(max(percentl))][2], finalline[percentl.index(max(percentl))][0], finalline[percentl.index(max(percentl))][-1]]
finalline.remove(finalline[percentl.index(max(percentl))])
percentl.remove(percentl[percentl.index(max(percentl))])
Z+=[finalline[percentl.index(max(percentl))][2], finalline[percentl.index(max(percentl))][0], finalline[percentl.index(max(percentl))][-1]]
finalline.remove(finalline[percentl.index(max(percentl))])
percentl.remove(percentl[percentl.index(max(percentl))])
Z+=[finalline[percentl.index(max(percentl))][2], finalline[percentl.index(max(percentl))][0], finalline[percentl.index(max(percentl))][-1]]
percentl.remove(percentl[percentl.index(max(percentl))])
